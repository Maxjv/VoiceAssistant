    ...
    if (contextModal && startContextBtn) {
        ...
        async function autoConnect() {
            try {
                ...
                if (localStorage.getItem('tfte_session_active') === 'true') {
                    ...
                    // Control de Aduana Silencioso (Apuntando al Agente real para atrapar el error 429)
                    fetch('/api/agente', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ message: "PING_CUOTA", backend: agentBackend, folder: '' })
                    }).then(res => {
                        if (!res.ok && (res.status === 401 || res.status === 403 || res.status === 429)) {
                            console.warn("Falta de cuota detectada al refrescar la página (F5).");
                            mostrarModalCreditos(agentBackend); // Disparamos el popup de falta de saldo
        ...
        startContextBtn.addEventListener('click', async () => {
            try {
                ...
                // 🔥 HARDCODE: Forzamos la ruta oficial de instalación como pediste
                const activeCtx = 'C:\\TFTE';
...
async function verificarCuotaYSaludar(esAuto) {
    ...
    try {
        ...
        // Hacemos un "Ping" rápido al backend del agente (Antigravity/Claude)
        const checkRes = await fetch('/api/agente', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ message: "PING_CUOTA", backend: agentBackend, folder: '' })
        });

        // Si el proveedor detecta falta de tokens, rebotará instantáneamente con 401, 403 o 429
        if (!checkRes.ok && (checkRes.status === 401 || checkRes.status === 403 || checkRes.status === 429)) {
            console.warn("Se detectó falta de cuota en el inicio de sesión.");
            mostrarModalCreditos(agentBackend); // Disparamos el popup que tenías desarrollado
            updateUIState('ready');
            return; // 🛑 BLOQUEAMOS EL SALUDO FALSO
        }
...
function logTaskToRepo(text, type = 'user') {
    ...
    card.innerHTML = `
        ...
        <div class="task-row" style="margin-top: 4px;">
            ...
            <div class="task-badge badge-pending" id="badge-conf-${id}" style="font-size: 0.6rem; padding: 2px 6px;">PENDING</div>
        </div>
    `;
    ...
    reloadSource.onmessage = (event) => {
        if (event.data === 'reload') {
            ...
            document.querySelectorAll('iframe').forEach(ifr => {
                ...
                ifr.src = 'about:blank';
                setTimeout(() => { ifr.src = currentSrc; }, 50);
            });
