...
if (process.platform === 'win32' && process.env.WATCHDOG_MANAGED !== 'true') {
    [4000].forEach(killPort);
}
// ---------------------------------------------------------
...
let isProjectCompiled = false;
let projectLaunchTime = Date.now();
...
function detectAndLaunchProject(startDir) {
    ...
    if (pkg) {
        ...
        projectLaunchTime = Date.now();

        const installCmd = fs.existsSync(path.join(finalDir, 'node_modules')) ? '' : 'npm install && ';
        let runCmd = '';
        if (deps['vite']) {
            targetPort = 5173;
            runCmd = scripts.dev ? 'npm run dev -- --host 127.0.0.1 --port 5173' : 'npm start';
        ...
        if (runCmd) {
            ...
            const checkCompiledOutput = (text) => {
                const clean = text.replace(/[\u001b\u009b][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><]/g, '');
                stdoutAccumulator = (stdoutAccumulator + clean).slice(-8192);
                if (/compiled successfully|compiled with warnings|webpack compiled|ready in|Local:\s*http|Servidor Web Interno listo/i.test(stdoutAccumulator)) {
                    if (!isProjectCompiled) {
                        isProjectCompiled = true;
                        console.log('[Sistema] ¡Proyecto compilado y listo para el usuario!');
                    }
                }
            };
            ...
            activeAppProcess.stderr.on('data', (d) => {
                const text = d.toString();
                console.error('[App]', text.trim());
                checkCompiledOutput(text);
            });

            activeAppProcess.on('exit', (code, signal) => {
                console.log(`[App] Proceso finalizado (código: ${code}, señal: ${signal})`);
                isProjectCompiled = false;
            });
        }
    }

    return { type: isNodeProject ? 'node' : 'web', port: isNodeProject ? targetPort : 8080, dir: finalDir };
}

const STATIC_WEB_PORT = 8080;
let staticWebServer = null;

function ensureStaticWebServer() {
    if (staticWebServer) return;
    const webApp = express();

    // Servir estáticos dinámicamente según el ROOT_DIR actual en cada request
    webApp.use((req, res, next) => {
        if (req.path === '/' || req.path === '') return next();

        const rawPath = decodeURIComponent(req.path.replace(/^\/+/, ''));
        const candidates = [
            path.join(ROOT_DIR, rawPath),
            path.join(ROOT_DIR, 'public', rawPath),
            path.join(ROOT_DIR, 'dist', rawPath)
        ];
    ...
    // Resolver HTML dinámico para la ruta raíz o rutas SPA
    webApp.use((req, res) => {
        let targetIndex = null;
        if (fs.existsSync(path.join(ROOT_DIR, 'index.html'))) {
            targetIndex = path.join(ROOT_DIR, 'index.html');
        } else if (fs.existsSync(path.join(ROOT_DIR, 'public', 'index.html'))) {
            targetIndex = path.join(ROOT_DIR, 'public', 'index.html');
        } else if (fs.existsSync(path.join(ROOT_DIR, 'dist', 'index.html'))) {
            targetIndex = path.join(ROOT_DIR, 'dist', 'index.html');
        } else {
            try {
                const files = fs.readdirSync(ROOT_DIR);
                const anyHtml = files.find(f => f.toLowerCase().endsWith('.html') && f.toLowerCase() !== 'project_control.html');
                if (anyHtml) targetIndex = path.join(ROOT_DIR, anyHtml);
            } catch (e) { }
        }

        if (targetIndex && fs.existsSync(targetIndex)) {
...
<body>
    ...
    try {
        killPort(STATIC_WEB_PORT);
        staticWebServer = webApp.listen(STATIC_WEB_PORT, '127.0.0.1', () => {
            console.log(`[Sistema] Servidor Web Interno listo en ${STATIC_WEB_PORT}`);
        });
    } catch (e) {
        console.error('Error arrancando servidor web interno:', e);
    }
}

// Iniciar siempre el servidor web estático en background para disponibilidad inmediata
ensureStaticWebServer();

const projectInfo = detectAndLaunchProject(ROOT_DIR);
let PROJECT_TYPE = projectInfo.type;
let TARGET_PORT = (PROJECT_TYPE === 'web') ? STATIC_WEB_PORT : projectInfo.port;
if (PROJECT_TYPE === 'web') {
    isProjectCompiled = true;
}
...
// Endpoint para el Splash y Radar: solo responde ready cuando Cloudflare y el proyecto están 100% listos
app.get('/api/ready-url', (req, res) => {
    let cfUrl = '';
    const urlPath = path.join(BASE_DIR, 'current-url.txt');
    if (fs.existsSync(urlPath)) {
        try { cfUrl = fs.readFileSync(urlPath, 'utf8').trim(); } catch (e) { }
    }
    if (!cfUrl || !cfUrl.startsWith('http')) {
        const errPath = path.join(BASE_DIR, 'cloudflared.err.log');
        if (fs.existsSync(errPath)) {
            try {
                const log = fs.readFileSync(errPath, 'utf8');
                const m = log.match(/https:\/\/[a-z0-9-]+\.trycloudflare\.com/g);
                if (m && m.length > 0) {
                    cfUrl = m[m.length - 1].trim();
                    try { fs.writeFileSync(urlPath, cfUrl); } catch (e) { }
                }
            } catch (e) { }
        }
    }
    ...
    if (!cfUrl || !cfUrl.startsWith('http')) {
        return res.json({ ready: false, reason: 'Waiting for Cloudflare' });
    }
    ...
    onError: (err, req, res) => {
        if (!res.headersSent) {
            ...
            if (isHtml) {
                ...
                res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
                res.end(`<!DOCTYPE html><html><body style="background:#0f172a;color:#fff;"><script>setTimeout(() => location.reload(), 1000);</script></body></html>`);
            } else {
...
app.post('/api/set-context', (req, res) => {
    ...
    try {
        ...
        let projectType = 'web';
        if (newProj.type === 'node' && newProj.dir) {
            try {
                const pkgPath = path.join(newProj.dir, 'package.json');
...
const server = app.listen(port, '127.0.0.1', () => {
    ...
    if (process.platform === 'win32') {
        ...
        // 1. NODE ENCIENDE CLOUDFLARE SOLO EN DESARROLLO SIN WATCHDOG (EN PRODUCCIÓN O CON WATCHDOG LO GESTIONA WATCHDOG)
        if (process.env.ENV !== 'production' && process.env.WATCHDOG_MANAGED !== 'true') {
