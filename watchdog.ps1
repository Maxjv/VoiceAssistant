...
function Start-Cloudflared {
    ...
    $proc = Start-Process -FilePath $cfPath -ArgumentList "tunnel", "--no-autoupdate", "--metrics", "127.0.0.1:0", "--url", "http://127.0.0.1:$Port" -WorkingDirectory $root `
...
while ($true) {
    ...
    $cfNeedsRestart = $false
    if (-not (Test-Alive $cfPid)) {
        $cfNeedsRestart = $true
    } else {
        $errFile = Join-Path $root "cloudflared.err.log"
        if (Test-Path $errFile) {
            $lastErrors = Get-Content $errFile -Tail 10 -ErrorAction SilentlyContinue
            if ($lastErrors -match "failed to serve tunnel connection" -or $lastErrors -match "control stream encountered a failure" -or $lastErrors -match "Serve tunnel error") {
                Log-Msg "Detectada desconexion critica en tunel de Cloudflare. Reiniciando proceso..."
                $cfNeedsRestart = $true
            }
        }
    }
