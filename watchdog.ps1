...
function Start-ClaudeWatcher {
    return (Start-Process -FilePath "node" -ArgumentList "watcher\watch-claude.js" -WorkingDirectory $root -WindowStyle Hidden -PassThru).Id
}

function Start-GeminiWatcher {
    return (Start-Process -FilePath "node" -ArgumentList "watcher\watch-gemini.js" -WorkingDirectory $root -WindowStyle Hidden -PassThru).Id
}
...
Write-Host ">>> Si la sesión caducó, se abrirá el navegador para que autorices. <<<" -ForegroundColor Yellow
try {
    # Un ping ultra simple para despertar a agy y forzar el login si está vencido
    # Agregamos --dangerously-skip-permissions para que no pida confirmación de lectura de carpetas
    agy -p "Ping de sistema, responde con una sola palabra: OK" --mode plan --output-format text --dangerously-skip-permissions | Out-Null
    Log-Msg "Sesión de IA verificada correctamente."
} catch {
...
while ($true) {
    ...
    if (-not (Test-Alive $cfPid))     { Log-Msg "Reviviendo Cloudflared..."; $cfPid = Start-Cloudflared }
}